...
/// Task list categorization for GTD-style workflow
enum TaskList: String, Codable, CaseIterable {
    case inbox = "Inbox"
    case today = "Today"
...
struct EventTask: Identifiable, Codable, Hashable {
    ...
    // MARK: - Phase 1 Extensions (Hybrid Task System)

    /// Optional event linking - nil means standalone task
    var linkedEventId: String?

    /// Duration for time-blocking on calendar (in seconds)
    var duration: TimeInterval?

    /// Task list categorization for GTD workflow
    var taskList: TaskList?

    // MARK: - Phase 3 Extensions (Time-Blocking)

    /// Scheduled time for task on calendar (nil = not scheduled)
    var scheduledTime: Date?

    // MARK: - Phase 5 Extensions (Smart Automation)
...
class EventTaskManager: ObservableObject {
    ...
    func addTask(_ task: EventTask, to eventId: String) {
        ...
        var taskToAdd = task
        if taskToAdd.linkedEventId == nil {
            taskToAdd.linkedEventId = eventId
        }

        if var tasks = eventTasks[eventId] {
            tasks.tasks.append(taskToAdd)
        ...
        if settings.enableNotifications, taskToAdd.dueDate != nil, !taskToAdd.isCompleted {
            scheduleTaskNotification(for: taskToAdd, eventId: eventId)
        }
    }
    ...
    /// Get tasks filtered by TaskList category
    func getTasksForList(_ list: TaskList) -> [EventTask] {
        ...
        switch list {
        case .inbox:
            // Tasks without due dates or taskList explicitly set to inbox
            return allTasks.filter { task in
                !task.isCompleted && (task.dueDate == nil || task.taskList == .inbox)
            }

        case .today:
            // Tasks due today or taskList explicitly set to today
            return allTasks.filter { task in
                if task.isCompleted { return false }
                if task.taskList == .today { return true }
        ...
        case .upcoming:
            ...
            return allTasks.filter { task in
                if task.isCompleted { return false }
                if task.taskList == .upcoming { return true }
                if let dueDate = task.dueDate {
                    let isAfterToday = dueDate > today
                    let isWithinWeek = dueDate <= calendar.date(byAdding: .day, value: 7, to: today)!
                    return isAfterToday && isWithinWeek && !calendar.isDate(dueDate, inSameDayAs: today)
                }
                return false
            }

        case .someday:
            // Tasks far in the future or explicitly marked as someday
            return allTasks.filter { task in
                if task.isCompleted { return false }
                if task.taskList == .someday { return true }
                if let dueDate = task.dueDate {
                    return dueDate > calendar.date(byAdding: .day, value: 7, to: today)!
                }
                return false
            }
        }
    ...
    /// Get count of tasks for a specific list
    func getTaskCountForList(_ list: TaskList) -> Int {
        return getTasksForList(list).count
    }

    /// Get all standalone tasks (not linked to any event)
    func getStandaloneTasks() -> [EventTask] {
        return getAllTasks().filter { $0.linkedEventId == nil }
    }

    /// Get all event-linked tasks
    func getEventLinkedTasks() -> [EventTask] {
        return getAllTasks().filter { $0.linkedEventId != nil }
    }

    /// Update task's TaskList category
    func updateTaskList(_ taskId: UUID, to list: TaskList, in eventId: String) {
        guard var tasks = eventTasks[eventId] else { return }

        if let index = tasks.tasks.firstIndex(where: { $0.id == taskId }) {
            tasks.tasks[index].taskList = list
            eventTasks[eventId] = tasks
            saveToStorage()
        }
    }

    // MARK: - Phase 3: Time-Blocking

    /// Get all scheduled tasks for a specific date
    func getScheduledTasks(for date: Date) -> [(task: EventTask, eventId: String)] {
        let calendar = Calendar.current
        var scheduledTasks: [(task: EventTask, eventId: String)] = []

        for (eventId, eventTasksContainer) in eventTasks {
            for task in eventTasksContainer.tasks {
                if let scheduledTime = task.scheduledTime,
                   calendar.isDate(scheduledTime, inSameDayAs: date),
                   !task.isCompleted {
                    scheduledTasks.append((task, eventId))
                }
            }
        }

        return scheduledTasks.sorted { $0.task.scheduledTime! < $1.task.scheduledTime! }
    }

    /// Schedule a task for a specific time
    func scheduleTask(_ taskId: UUID, at time: Date, in eventId: String) {
        guard var tasks = eventTasks[eventId] else { return }

        if let index = tasks.tasks.firstIndex(where: { $0.id == taskId }) {
            tasks.tasks[index].scheduledTime = time
            eventTasks[eventId] = tasks
            saveToStorage()
    ...
    func unscheduleTask(_ taskId: UUID, in eventId: String) {
        guard var tasks = eventTasks[eventId] else { return }

        if let index = tasks.tasks.firstIndex(where: { $0.id == taskId }) {
            tasks.tasks[index].scheduledTime = nil
            eventTasks[eventId] = tasks
            saveToStorage()
            objectWillChange.send()
        }
    ...
    func rescheduleTask(_ taskId: UUID, to newTime: Date, in eventId: String) {
        scheduleTask(taskId, at: newTime, in: eventId)
    }

    // MARK: - Phase 5: Smart Automation

    /// Enable auto-schedule for optimal time slot finding
    func enableAutoSchedule(for taskId: UUID, in eventId: String) {
        guard var tasks = eventTasks[eventId] else { return }

        if let index = tasks.tasks.firstIndex(where: { $0.id == taskId }) {
            tasks.tasks[index].autoSchedule = true
            eventTasks[eventId] = tasks
            saveToStorage()
            objectWillChange.send()
            print("✅ Enabled auto-schedule for task")
        }
    }

    /// Get all tasks created from a specific template
    func getTasksFromTemplate(_ templateId: UUID) -> [(task: EventTask, eventId: String)] {
        var results: [(task: EventTask, eventId: String)] = []

        for (eventId, taskGroup) in eventTasks {
            for task in taskGroup.tasks where task.templateId == templateId {
                results.append((task, eventId))
            }
        }
    ...
    /// Get task completion history for pattern detection
    func getTaskHistory() -> [EventTask] {
        var history: [EventTask] = []

        for taskGroup in eventTasks.values {
            history.append(contentsOf: taskGroup.tasks)
        }
